...
namespace SwissTransport
{
    public class Transport : ITransport
    {
        ...
        public Connections GetConnectionByDateTime(string fromStation, string toStation, string date, string time, short isArrival)
        {
            if(time=="")
            {
                time = "00:00";
            ...
            fromStation = System.Uri.EscapeDataString(fromStation);
            toStation = System.Uri.EscapeDataString(toStation);
            var request = CreateWebRequest("http://transport.opendata.ch/v1/connections?from=" + fromStation + "&to=" + toStation +"&date=" + date + "&time=" + time + "&isArrivalTime=" + isArrival.ToString());
            var response = request.GetResponse();
            var responseStream = response.GetResponseStream();
